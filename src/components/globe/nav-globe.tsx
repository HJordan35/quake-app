import { useRef, useState, useCallback, useEffect, forwardRef, ForwardedRef } from 'react'
...
const NavGlobe = forwardRef(
  ({ data = [], onPointClick, width, height, className = '' }: NavGlobeProps, ref: ForwardedRef<GlobeMethods>) => {
    const innerGlobeRef = useRef<GlobeMethods>(null as unknown as GlobeMethods)
    const containerRef = useRef<HTMLDivElement>(null)
    const [pointData, setPointData] = useState<PointData[]>([])
    const [dimensions, setDimensions] = useState({ width: 0, height: 0 })

    // Expose the globe methods to the parent component via the forwarded ref
    useEffect(() => {
      if (ref && innerGlobeRef.current) {
        if (typeof ref === 'function') {
          ref(innerGlobeRef.current)
        } else {
          ref.current = innerGlobeRef.current
        }
      }
    }, [ref, innerGlobeRef.current])
    ...
    const handlePointClick = useCallback(
      (point: object) => {
        ...
        if (innerGlobeRef.current) {
          innerGlobeRef.current.pointOfView(
            {
              lat: typedPoint.lat,
              lng: typedPoint.lng,
              altitude: 1.5,
